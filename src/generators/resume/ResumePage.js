    ...
    if (isSidebar) {
      ...
      html = `<div style="font-family:${t.font};display:flex;min-height:100%;">
              ...
              <div style="flex:1;padding:16px;">
                ...
                ${education.length > 0 ? `<div style="margin-bottom:14px;">${sectionHeading('Education')}${education.map(e => `<div style="margin-bottom:6px;"><div style="display:flex;justify-content:space-between;"><strong style="font-size:11px;">${e.eduDegree || ''}</strong><span style="font-size:9px;color:#888;">${e.eduYear || ''}</span></div><div style="font-size:10px;color:#666;">${e.eduSchool || ''} ${e.eduGrade ? `— ${e.eduGrade}` : ''}</div></div>`).join('')}</div>` : ''}
                ${highschool.length > 0 ? `<div style="margin-bottom:14px;">${sectionHeading('High School')}${highschool.map(h => `<div style="margin-bottom:6px;"><div style="display:flex;justify-content:space-between;"><strong style="font-size:11px;">${h.hsName || ''}</strong><span style="font-size:9px;color:#888;">${h.hsYear || ''}</span></div><div style="font-size:10px;color:#666;">${h.hsSubjects || ''}</div></div>`).join('')}</div>` : ''}
              </div>
            </div>`;
    } else if (isDark) {
      // Dark header layout
      html = `<div style="font-family:${t.font};">
              ...
              ${data.summary ? `<div style="margin-bottom:14px;">${sectionHeading('Summary')}<p style="font-size:10px;color:#444;line-height:1.6;">${data.summary}</p></div>` : ''}
              ${experience.length > 0 ? `<div style="margin-bottom:14px;">${sectionHeading('Experience')}${experience.map(e => `<div style="margin-bottom:8px;"><div style="display:flex;justify-content:space-between;"><strong style="font-size:11px;">${e.expTitle || ''}</strong><span style="font-size:9px;color:#888;">${e.expStart || ''} — ${e.expEnd || ''}</span></div><div style="font-size:10px;color:${t.accent};">${e.expCompany || ''}</div><p style="font-size:9px;color:#555;margin-top:3px;">${e.expDesc || ''}</p></div>`).join('')}</div>` : ''}
              ${education.length > 0 ? `<div style="margin-bottom:14px;">${sectionHeading('Education')}${education.map(e => `<div style="margin-bottom:6px;"><div style="display:flex;justify-content:space-between;"><strong style="font-size:11px;">${e.eduDegree || ''}</strong><span style="font-size:9px;color:#888;">${e.eduYear || ''}</span></div><div style="font-size:10px;color:#666;">${e.eduSchool || ''}</div></div>`).join('')}</div>` : ''}
              ${highschool.length > 0 ? `<div style="margin-bottom:14px;">${sectionHeading('High School')}${highschool.map(h => `<div style="margin-bottom:6px;"><div style="display:flex;justify-content:space-between;"><strong style="font-size:11px;">${h.hsName || ''}</strong><span style="font-size:9px;color:#888;">${h.hsYear || ''}</span></div><div style="font-size:10px;color:#666;">${h.hsSubjects || ''}</div></div>`).join('')}</div>` : ''}
